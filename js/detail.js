document.addEventListener("DOMContentLoaded", () => {
    ...
    async function loadProjectDetails() {
        try {
            ...
            safeSetText("pjComments", project.comments ?? Math.floor(Math.random() * 12) + 3);

            // Like Button UI အခြေအနေကို Update လုပ်ခြင်း
            updateLikeUI();

            const avatarEl = document.getElementById("pjAvatar");
            if (avatarEl) avatarEl.src = project.userAvatar || "https://via.placeholder.com/40";
            
            const imgEl = document.getElementById("pjImage");
            if (imgEl) imgEl.src = project.image || "https://via.placeholder.com/800x400";
            ...
            if (wiringEl) {
                wiringEl.innerHTML = project.wiringNotes 
                    ? project.wiringNotes.split('\n').map(line => `<li class="mb-2"><i class="bi bi-arrow-right-short text-warning me-1"></i>${line}</li>`).join('') 
                    : "<li>No wiring notes provided.</li>";
            }

            safeSetText("pjCode", project.sourceCode || "// No code available");
